...
\section{Interacción de dos vientos: Aproximación Hipersónica \citep{Canto:1996}}
...
Definimos \dot{M}(\theta), \vec{\dot{\Pi}}(\theta) y \dot{J}(theta) como la tasa de pérdida de masa, la
tasa de momento y la tasa de momento angular, respectivamente, de la capa delgada integradas
desde $\theta=0$ hasta $\theta$. Éstas se calculan de la siguiente manera:

\begin{align}
  \vec{\dot{\Pi}}(\theta) &= \dot{\Pi}_r(\theta) \hat{r} + \dot{\Pi}_z(\theta) \hat{z} = \dot{M}\left(
                      v_r \hat{r} + v_z\hat{z}\right) \\
...
\begin{align}
  \dot{J}(\theta) &= \dot{M}(\theta)R(\theta)v_\theta \\
  \mathrm{donde:} & v_\theta = v_r\cos\theta - v_z\sin\theta
\end{align}
