...
\frame{\frametitle{Motivation of this work}
\twocols{We do this by measuring two non-dimensional parameters: the planitude $\Pi=R_c/R_0$ and the alatude $\Lambda=R_{90}/R_0$, easily measurable from observations.}{\includegraphics[width=\linewidth]{../Figures/radii-scheme}}
}
...
\frame{\frametitle{Bow Shocks in the ISM}
  \twocols{\begin{block}{}
      ...
      \begin{itemize}
      \item \textbf{Jets working surfaces}
...
\frame{\frametitle{Proplyds in Orion Nebula}
  \twocolspic{\begin{block}{}About 70 arcs have been detected within Orion Nebula \citep{Gutierrez-Soto:2015a}, many of them are produced by proplyds.\end{block}}{\begin{block}{}\includegraphics[width=\linewidth]{../Figures/ll-pos-image-Luis} \\
...
\frame{
  \twocolspic{\begin{block}{}
      The head is shaped by the incident UV radiation from the massive star, forming a D type Ionization Front, while the tail is shaped by diffuse radiation.
    \end{block}
}{ \begin{block}{}
...
\frame[label=generic-bow]{\frametitle{General Considerations}
  ...
  \end{block}
  \twocolspic{\begin{block}{}
      \only<1>{A weaker wind placed at the origin, and a stronger wind separated by a distance $D$ from the former one.}
...
\frame{\frametitle{Bow shocks terminology}
\twocolspic{\begin{itemize}
	\item The minimal radius of the bow shock is called ``head'' or apex.
	\item The far sides of the apex are called ``wings''
\end{itemize}}{\includegraphics[width=\linewidth]{../Figures/bow-terminology}}
}
\frame[label=Pi-Lambda]{\frametitle{Planitude and Alatude}
  ...
  \twocols{\begin{block}{}
      ...
      \item \textbf<3>{The asymptotic angle $\theta_\infty$ of the far wings (which usually is not measurable)}
...
\frame[label=quadrics]{\frametitle{Quadrics of Revolution: basic definitions}
  \begin{block}{}
...
\end{block}
\hyperlink{parametrization}{\beamergotobutton{To see the parametrization of quadrics click HERE}}
}
...
\frame{\frametitle{Quadrics of revolution: basic definitions}
\begin{block}{}
	This implies that the relative values of $(\Pi, \Lambda)$ determines the type of conic.
\end{block}
\twocols{\begin{block}{\only<1>{$2\Pi > \Lambda^2$}\only<2>{2\Pi=\Lambda^2}}
	\centering
	\includegraphics<1>[width=0.5\linewidth]{../Figures/ellipsoid}
	\includegraphics<2>[width=0.5\linewidth]{../Figures/paraboloid}
\end{block}}{\begin{block}{\only<1>{2\Pi - \Lambda^2 = 1}\only<2>{2\Pi < \Lambda^2}}
	\centering
	\includegraphics<1>[width=0.5\linewidth]{../Figures/spheroid}
	\includegraphics<2>[width=0.5\linewidth]{../Figures/hyperboloid}
\end{block}}
%{\twocols{\begin{block}{
%	%\end{block}}{\begin{block}{}
%	%\end{block}}}
...
\frame[label=3d-shape]{\frametitle{Projection Onto the Plane of Sky}
  \begin{block}{\small Tridimensional shape in the body frame: Rotation along the $x$ axis of the bidimensional curve.}
    \includegraphics[width=0.7\linewidth]{../Figures/projection-3d-shape-eq} \\
  \end{block}
  \begin{block}{\small Transformation between body frame and observer frame}
    \includegraphics[width=0.7\linewidth]{../Figures/projection-rotation-eq} \\
...
\frame[label=tangent-line]{\frametitle{Projection Onto the Plane of Sky}
  \twocols{\begin{block}{Normal and tangent unit vectors}
      We define the unit vectors $\hat{t}$ and $\hat{n}$ as the tangent and normal vectors to the surface at certain point (to check the expression of each one go to \hyperlink{unit-vec}{\beamergotobutton{this slide}}). The tangent line of the surface at the inclination $i$ is such that the line of sight is othogonal to the normal unitary vector:
      \begin{align*}
        \hat{n}\cdot\hat{z}' = 0
      \end{align*}
    \end{block}}{\begin{block}{}
...
\frame{\frametitle{Projection Onto the Plane of Sky}
  \twocols{\begin{block}{}
    ...
    \end{block}}
    \begin{block}{}
	    Since $z'_T$ is not a linear function of $\theta$, then the projected shape does not lie in a plane.
    \end{block}
}

\frame[label=proj-pi-lambda]{\frametitle{Projected Alatude and Planitude}
\begin{block}{}
...
\begin{block}{}
  \begin{itemize}
  \item The projected radius at the apex $R'_0$ is computed as $x'_T$ such that $y'_T=0$:
  \item $R'_{90}$ is computed as $y'_T$ such that $x'_T=0$
  \item And the projected radius of curvature is computed as the intrinsic planitude but in the observer frame.
    \hyperlink{proj-radii}{\beamergotobutton{To see the equations for each radii, click HERE}}
  \end{itemize}
\end{block}}
...
%	\end{align*}
%\end{block}}{
  \begin{block}{Projected apex radius}
    \centering
	\includegraphics[width=0.6\linewidth]{../Figures/projected-R0-vs-i}
      \end{block}%}
}
...
%	\end{align*}
% \end{block}}{
  \begin{block}{Projected planitude}
    \centering
	\includegraphics[width=0.6\linewidth]{../Figures/projected-Rc-vs-i}
\end{block}%}
}
\frame{\frametitle{Projected Alatude and Planitude of Quadrics of Revolution}
...
% \end{block}}{
  \begin{block}{Projected alatude}
    \centering
	\includegraphics[width=0.6\linewidth]{../Figures/projected-R90-vs-i}
\end{block}%}
}
...
\frame{\frametitle{Planitude and alatude}
  \begin{block}{True $\Lambda-\Pi$ diagram}
    \centering
    \includegraphics[width=0.9\linewidth]{../Figures/Pi-vs-Lambda}
  \end{block}
}
%\frame{\frametitle{Planitude and Alatude}
...
\twocolspic{With DS9 tools, we get the coordinates (AR, DEC) of each proplyd (small circles), the position of \thC{} (red cross) and the position of each bow shock (cyan crosses).}{\begin{block}{}
	\includegraphics[width=\linewidth]{../Figures/Trapezium-annotate-rob-2018}
\end{block}}
}
\frame{\frametitle{Obtaining the data}
...
\begin{block}{}
	\begin{itemize}
		...
		\item Improve methodology to evaluate balance conditions.
                  \item Analyze the $\beta\to 0$ limit for ancantoids which may apply for jets working surfaces.
...
\frame{\frametitle{Summary and conclusions}
  \small
  \begin{itemize}
  \item We evaluated ionization balance and ram pressure balance under certain assumptions about \thC{} wind properties.
  \item We assumed that the photoevaporated flow velocity lies in the range $M=[2, 3]$, we didn't see sustancial changes in both balance conditions.
    \item We compared two different estimations of the photoevaporated flow density and evaluated balance conditions, but we lack of an appropiate method to determine which approximation is better (or if none of them is).
    \end{itemize}
}
...
\frame[label=parametrization]{\frametitle{Quadrics of Revolution: Parametrization}
...
\frame[label=unit-vec]{\frametitle{Useful unitary vectors}
 ...
 \frame[label=proj-radii]{\frametitle{Projected Alatude and Planitude}
	\begin{align*}
		R'_0 = R(\theta_0)\cos(\theta_0 - |i|)
	\end{align*}
        \twocols{\begin{block}{Projected alatude}
\small            \includegraphics[width=0.8\linewidth]{../Figures/projection-3d-R90-eq}\\
	Where:\\
	\includegraphics[width=0.8\linewidth]{../Figures/projection-3d-t90-eq}\\
	Finally $\Lambda = R'_{90}/R'_0$
\end{block}}{\begin{block}{projected planitude}
  \includegraphics[width=\linewidth]{../Figures/projection-3d-planitude-eq}\\
  \hyperlink{proj-pi-lambda}{\beamergotobutton{Return Button}}
 \end{block}}
