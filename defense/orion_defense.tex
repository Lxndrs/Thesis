...
\frame[label=shape]{\frametitle{Discontinuity Contact Shape}
  \twocols{\begin{block}{}
	  \small
      The shape of the dicontinuity contact $R(\theta)$ is give by equation (6) of \citep{Canto:1996}:\\
...
\frame{\frametitle{Projected Planitude and Alatude: $\Lambda-\Pi$ diagram}
\twocols{\begin{block}{}
	...
	\only<2>{The ancantoids show a ``kink'' at an inclination shuch that the apparent apex passes through the inner wind discontinuity, where the second derivative of $R(\theta)$ has a slope deviation. This kink softens as the anisotropy parameter increases.}
...
\frame{\frametitle{MPAQ (Most Probably Asked Questions)}}
...
\frame{\frametitle{Anosotropy index for proplyds}
\begin{block}{}
